...
- **Regression tests:** `test/agent-browser.results.test.ts` locks classifier and next-action behavior; `test/agent-browser.extension-validation.test.ts` asserts `details` on representative tool outcomes and includes deterministic fake-upstream matrices for `--json` plus implicit `--session` argv ordering (`connect 9222`, `download` with a selector and path, `get url`, `snapshot --compact`, `tab new` / `tab 0` / `tab close`) and for non-core command families (network, diff, trace/profiler/record, console/errors/highlight/inspect/clipboard, stream, dashboard, chat) (complementing the slower real-browser contract in `test/agent-browser.real-upstream-contract.test.ts`); `test/agent-browser.presentation.test.ts` covers batch and presentation wiring.
