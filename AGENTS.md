...
**npm verification facade:** `npm run verify` and `npm run docs` delegate to [`scripts/project.mjs`](scripts/project.mjs) (`verifySteps` / `docsSteps`). The default gate runs generated playbook drift, `tsc --noEmit`, `tsx --test test/**/*.test.ts`, command-reference generated-block checks, then live `scripts/verify-command-reference.mjs` (needs the baseline-matching `agent-browser` on `PATH`). `release` concatenates the default steps with `verify-package.mjs --smoke-pi`. [`test/project-verify.test.ts`](test/project-verify.test.ts) locks default, `release`, `real-upstream`, `package-pi`, and combined-docs orchestration so those steps cannot be dropped silently. Run `npm run verify -- --help` for opt-in modes and supported passthrough flags.
...
- **Regression tests:** `test/agent-browser.extension-validation.test.ts` asserts compiled argv and pre-spawn validation errors for `semanticAction`, including mutual exclusion with `job`, `qa`, and `sourceLookup`.

### Constrained `job` (top-level tool input)

- **Compilation and validation:** `compileAgentBrowserJob` in `extensions/agent-browser/index.ts` turns the optional `job` object into upstream `batch` argv plus JSON-array stdin; it is mutually exclusive with `args`, `semanticAction`, `qa`, and `sourceLookup` on the same call and rejects caller-provided `stdin` alongside `job`. Changing allowed step actions, fields, or argv shape requires updating that helper and `AGENT_BROWSER_JOB_STEP_ACTIONS`, the TypeBox schema on `AGENT_BROWSER_PARAMS`, the human contract in [`docs/TOOL_CONTRACT.md`](docs/TOOL_CONTRACT.md#job), examples in [`docs/COMMAND_REFERENCE.md`](docs/COMMAND_REFERENCE.md) and [`README.md`](README.md), playbook strings in `extensions/agent-browser/lib/playbook.ts` (then `npm run docs -- playbook write`), the efficiency benchmark scenario set in `scripts/agent-browser-efficiency-benchmark.mjs` when adding representative job shapes, and `test/agent-browser.extension-validation.test.ts` / `test/agent-browser.efficiency-benchmark.test.ts`.
...
- **Regression tests:** `test/agent-browser.extension-validation.test.ts` covers compilation, redaction of secrets inside compiled stdin, mutual exclusion with `args` / `semanticAction` / `sourceLookup` / caller `stdin`, and invalid steps.

### Lightweight `qa` preset (top-level tool input)

- **Compilation and validation:** `compileAgentBrowserQaPreset` in `extensions/agent-browser/index.ts` builds a fixed diagnostic `batch` plan (buffer clears, `open`, `wait --load networkidle`, optional text/selector waits, optional `network requests` / `console` / `errors` reads, optional `screenshot`) from the `qa` object; it is mutually exclusive with `args`, `semanticAction`, `job`, and `sourceLookup` on the same call and rejects caller-provided `stdin` alongside `qa`. Changing fields, default checks, or compiled argv requires updating that helper, the TypeBox schema on `AGENT_BROWSER_PARAMS`, the human contract in [`docs/TOOL_CONTRACT.md`](docs/TOOL_CONTRACT.md#qa), README and [`docs/COMMAND_REFERENCE.md`](docs/COMMAND_REFERENCE.md) examples, playbook strings in `extensions/agent-browser/lib/playbook.ts` (then `npm run docs -- playbook write`), the efficiency benchmark when adding representative `qa` shapes, and `test/agent-browser.extension-validation.test.ts` / `test/agent-browser.efficiency-benchmark.test.ts`. Downstream QA pass/fail classification lives in `analyzeQaPresetResults` in the same module; extending failure semantics requires updating [`docs/TOOL_CONTRACT.md`](docs/TOOL_CONTRACT.md#details) (`failureCategory: "qa-failure"`) and extension-validation expectations.
- **Runtime and presentation:** compiled QA presets use the same `batch` path and presentation as `job` until `analyzeQaPresetResults` may reclassify a successful upstream batch as a tool failure when diagnostics report console errors, page errors, or failed network requests.
- **Regression tests:** `test/agent-browser.extension-validation.test.ts` covers compilation, diagnostic failure classification, and mutual exclusion with other input modes and caller `stdin`.

### Experimental `sourceLookup` (top-level tool input)

- **Compilation and validation:** `compileAgentBrowserSourceLookup` in `extensions/agent-browser/index.ts` turns the optional `sourceLookup` object into upstream `batch` argv plus JSON-array stdin (steps such as `is visible`, `get html`, `react inspect`, `react tree` depending on which of `selector`, `reactFiberId`, and `componentName` are set). It is mutually exclusive with `args`, `semanticAction`, `job`, and `qa` on the same call and rejects caller-provided `stdin` alongside `sourceLookup`. Changing fields, compiled steps, or analysis rules requires updating that helper, `analyzeSourceLookupResults` in the same module, the TypeBox schema on `AGENT_BROWSER_PARAMS`, the human contract in [`docs/TOOL_CONTRACT.md`](docs/TOOL_CONTRACT.md#sourcelookup), examples in [`docs/COMMAND_REFERENCE.md`](docs/COMMAND_REFERENCE.md) and [`README.md`](README.md), playbook strings in `extensions/agent-browser/lib/playbook.ts` (then `npm run docs -- playbook write`), the efficiency benchmark when adding representative `sourceLookup` shapes, and `test/agent-browser.extension-validation.test.ts` / `test/agent-browser.efficiency-benchmark.test.ts`.
...
- **Launch-scoped flags** — `getStartupScopedFlags` mirrors `LAUNCH_SCOPED_FLAG_DEFINITIONS` (`--auto-connect`, `--cdp`, `--enable`, `--init-script`, `--profile`, `--session-name`, `--state`, `--provider`, `-p`, `--device`). `--auto-connect` counts only when enabled (bare or `true`); `--state` after a `wait` command is treated as a wait predicate, not launch state, so it does not force `sessionMode: "fresh"` during implicit reuse.
...
`scripts/agent-browser-efficiency-benchmark.mjs` is a **deterministic accounting benchmark**: it does not shell out to `agent-browser`, does not launch a browser, and does not read or write Pi sessions. It models representative `agent_browser` call shapes (including optional `stdin` for `batch` and top-level `job`, `qa`, or `sourceLookup` objects that compile to batch) and synthetic model-visible strings to produce comparable totals: scenario success rate, total tool calls, UTF-8 byte volume of model-visible output, stale-ref failure and recovery counts, artifact success count, distinct failure-category coverage, and summed elapsed-time estimates.
