...
- **Gate:** `npm run verify -- benchmark` runs the script once (JSON) plus `test/agent-browser.efficiency-benchmark.test.ts`. Default `npm run verify` does not include this slice; it stays opt-in so routine gates are not coupled to benchmark churn.
