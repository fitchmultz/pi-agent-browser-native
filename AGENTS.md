...
- **Stateful presentation:** successful tool and batch-step `details.data` pass through `redactStatefulPresentationData` in `presentation.ts` for `cookies` / `storage` (field-aware value redaction plus diagnostic line formatters), and through `redactSensitiveValue` for `auth`, `dialog`, `frame`, and `state` JSON. `redactInvocationArgs` in `runtime.ts` additionally replaces the cookie value token on `cookies set <name> <value> …` and the value token on `storage local|session set <key> <value> …` when building echoed argv and batch command text; failed batch steps run `redactExactValues` on structured errors so argv secrets are not repeated in error blobs. Changing any of this behavior requires updating [`docs/TOOL_CONTRACT.md`](docs/TOOL_CONTRACT.md#details), README agent rules, [`docs/COMMAND_REFERENCE.md`](docs/COMMAND_REFERENCE.md) operational notes, and presentation/runtime tests.
- **Regression tests:** `test/agent-browser.results.test.ts` locks classifier and next-action behavior; `test/agent-browser.extension-validation.test.ts` asserts `details` on representative tool outcomes and includes a deterministic fake-upstream matrix that checks `--json` plus implicit `--session` argv passthrough for commands such as `connect 9222`, `download` with a selector and path, `get url`, `snapshot --compact`, and `tab new` / `tab 0` / `tab close` (complementing the slower real-browser contract in `test/agent-browser.real-upstream-contract.test.ts`); `test/agent-browser.presentation.test.ts` covers batch and presentation wiring.
