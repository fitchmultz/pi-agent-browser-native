...
- **Regression tests:** `test/agent-browser.results.test.ts` locks classifier and next-action behavior; `test/agent-browser.extension-validation.test.ts` asserts `details` on representative tool outcomes and includes a deterministic fake-upstream matrix that checks `--json` plus implicit `--session` argv passthrough for commands such as `connect`, `download`, `get url`, `snapshot --compact`, and basic `tab` lifecycle steps (complementing the slower real-browser contract in `test/agent-browser.real-upstream-contract.test.ts`); `test/agent-browser.presentation.test.ts` covers batch and presentation wiring.
