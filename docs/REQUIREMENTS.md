...
- Local checkout hot-reload, exact-session relaunch, and branch rehydration validation should use configured-source lifecycle mode: exactly one active checkout/package source in Pi settings, launched with plain `pi` (or the lifecycle harness' exact `--session-id` relaunch path), so `/reload` and Pi session events exercise discovered/configured resources.
...
- Validate a full `pi` restart with exact `--session-id` relaunch or `/resume` when changes touch managed-session continuity, reload behavior, branch-backed state, or persisted artifact paths.
