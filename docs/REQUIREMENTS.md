...
- `qa.attached` must never erase existing session diagnostics before deciding pass/fail; URL-opening `qa` may clear buffers to scope a fresh page load, while attached QA preserves buffers and reports that scope in `details.compiledQaPreset.checks.diagnosticsResetAtStart`.
