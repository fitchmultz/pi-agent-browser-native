...
- Each tool invocation must supply **exactly one** of: `args` (full upstream argv after the binary name), top-level `semanticAction` (compiled into upstream `find` argv), constrained top-level `job`, lightweight `qa`, experimental `sourceLookup`, or experimental `networkSourceLookup`. Supplying none of these or more than one is rejected before launch (`extensions/agent-browser/index.ts`, `test/agent-browser.extension-validation.test.ts`).
- Optional caller `stdin` is supported only for upstream shapes that need it (for example `batch`, `eval --stdin`, `auth save --password-stdin`); `job`, `qa`, `sourceLookup`, and `networkSourceLookup` generate their own `batch` stdin and reject a caller-provided `stdin` alongside those modes.
...
- Supported `semanticAction` actions, locators, exclusivity rules, when `details.compiledSemanticAction` appears, and bounded `try-*-candidate` follow-ups on `selector-not-found` (specific action/locator pairs only; see contract) are specified in [`TOOL_CONTRACT.md`](TOOL_CONTRACT.md#semanticaction), with workflow examples in [`COMMAND_REFERENCE.md`](COMMAND_REFERENCE.md). Constrained `job`, `qa`, `sourceLookup`, and `networkSourceLookup` contracts live in [`TOOL_CONTRACT.md`](TOOL_CONTRACT.md#job), [`TOOL_CONTRACT.md`](TOOL_CONTRACT.md#qa), [`TOOL_CONTRACT.md`](TOOL_CONTRACT.md#sourcelookup), and [`TOOL_CONTRACT.md`](TOOL_CONTRACT.md#networksourcelookup).
...
- Successful and failed tool outcomes should surface bounded machine-readable fields on Pi-facing `details` (`resultCategory`, `successCategory`, `failureCategory`, optional structured `nextActions`, optional `pageChangeSummary` with per-step summaries on `batch`, optional `artifactVerification` with the same shape on successful `batchSteps[]` rows, plus optional diagnostics such as `refSnapshot` for page-scoped `@e…` guards, `overlayBlockers`, `selectorTextVisibility` / `selectorTextVisibilityAll`, `evalStdinHint`, `timeoutPartialProgress` after wrapper watchdog timeouts, `managedSessionOutcome` when a fresh managed-session plan fails, `artifactCleanup` after successful `close` with manifest entries, and compiled echoes like `compiledJob` / `compiledSemanticAction` / `compiledSourceLookup` / `compiledNetworkSourceLookup` where applicable) so agents can branch without parsing prose; stateful commands (`auth`, `cookies`, `storage`, `dialog`, `frame`, `state`) plus other structured diagnostics (for example `network`, `diff`, `trace`, `stream`, `dashboard`, `chat`) and `batch` should redact secret-bearing payloads in model-facing `details.data`, including the compact per-step `batch` roll-up on the parent result (full per-step payloads live on `batchSteps[]`). The contract lives in [`TOOL_CONTRACT.md`](TOOL_CONTRACT.md#details), enums and classifier precedence live in `extensions/agent-browser/lib/results/shared.ts`, and presentation-time summaries, redaction, and artifact verification rollups are assembled in `extensions/agent-browser/lib/results/presentation.ts` (`buildPageChangeSummary`, `PAGE_CHANGE_SUMMARY_COMMANDS`, `redactPresentationData`, `buildArtifactVerificationSummary`, `buildBatchPresentation`).
