...
- Only `electron.list` avoids invoking upstream `agent-browser`; `launch`, `status`, `probe`, and `cleanup` use the same binary resolution as other tool calls. A missing `agent-browser` on `PATH` therefore surfaces as `failureCategory: "missing-binary"` for those actions while `list` can still return catalog metadata.
