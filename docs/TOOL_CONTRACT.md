...
- keep runtime cleanup ownership separate from branch-visible state: `session_tree` may switch the visible branch, but extension-managed sessions and wrapper-launched Electron records owned by the current process remain eligible for quit/cleanup, and fresh-session allocation stays monotonic across branch restores
