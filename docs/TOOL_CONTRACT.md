...
`overlayBlockers` may appear after a successful direct `click` only when the wrapper's before/after URL stayed the same and a fresh post-click `snapshot -i` contains both overlay context (for example a `dialog`/`alertdialog` role or banner/modal/donation/cookie/consent text) and a visible close-like `button`, `link`, or `menuitem`. The field contains a short `summary` plus bounded `candidates` with `ref`, optional `role`/`name`, exact `args`, and evidence `reason`; visible text includes the same `Possible overlay blockers` block. This is conservative diagnostic evidence, not proof that the candidate should be clicked; prefer `inspect-overlay-state` first unless the close/dismiss target is clearly safe.
...
- `overlayBlockers` for conservative post-click overlay/banner/dialog blocker candidates when a direct click stays on the same URL and a fresh snapshot provides evidence
