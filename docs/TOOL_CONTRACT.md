...
- treat explicit caller-provided non-managed `--session` choices as user-managed; those names alone isolate a live browser session but are not a persisted tab/auth restore mechanism after a close command (`close`, `quit`, or `exit`), so use `--session <id> --restore`, `--profile`, or `--state` when persisted auth/tab state is required. Wrapper-owned managed sessions (only when the extension injects/rotates the managed session, not merely because a name starts with `piab-`) set a cwd-stable `AGENT_BROWSER_RESTORE` env key automatically (disable with `PI_AGENT_BROWSER_MANAGED_SESSION_RESTORE=0`) so SSO cookies/localStorage/sessionStorage can survive idle shutdown and later chats in the same project without a manual `state save` flow. Project/user/explicit upstream config with profile/cdp/provider/state/containment defaults blocks automatic restore. On POSIX the wrapper enforces owner-only mode `0700` on `~/.agent-browser` before automatic restore; Windows requires `AGENT_BROWSER_ENCRYPTION_KEY`. When a managed session was launched with an incompatible path, `details.managedSessionRestoreDisabled: true` means later bare follow-ups will not inject restore for that session identity
