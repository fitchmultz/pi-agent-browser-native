...
- `timeoutPartialProgress` after a wrapper watchdog timeout on a compiled `job` or `batch` when best-effort recovery found useful context; includes `steps` (planned args with 1-based indices), `currentPage` from session-scoped `get url` / `get title` when available, `artifacts` for declared `screenshot`, `pdf`, `download`, or `wait --download` paths with absolute path, existence, size, and step index, plus a summary string. This is recovery evidence only; missing entries do not prove the upstream step failed before creating other side effects.
...
- keep wrapper-spawned commands below the upstream CLI IPC read-timeout budget by clamping `AGENT_BROWSER_DEFAULT_TIMEOUT` to 25 seconds and stopping a stuck child process before the upstream 30-second retry path begins; timed-out `job` / `batch` calls may add `details.timeoutPartialProgress` and visible `Timeout partial progress` evidence with planned steps, current page title/URL, and declared artifact path checks
