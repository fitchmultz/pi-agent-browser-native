...
- Always close browser work with close/quit/exit when finished. The wrapper tracks managed and explicit sessions opened through agent_browser and best-effort closes them on Pi quit; idle timeout (default 15 minutes via AGENT_BROWSER_IDLE_TIMEOUT_MS / PI_AGENT_BROWSER_IMPLICIT_SESSION_IDLE_TIMEOUT_MS) is the crash backstop. pi-agent-browser-doctor warns about live /tmp/piab*.pid daemons.
