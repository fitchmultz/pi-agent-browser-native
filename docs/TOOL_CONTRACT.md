...
- `select` accepts either non-empty `selector` plus `value`/`values`, or locator fields that the wrapper resolves to exactly one current visible combobox/listbox `@ref` on an active session. Locator forms: `{ action: "select", locator: "role", role: "combobox", name: "Flavor", value: "chocolate" }` (option in `value` when `role` is set) or `{ action: "select", locator: "label", value: "Flavor", values: ["chocolate"] }` (control text in `value`, options in `values`). `select` does not accept `text`, and `selector` cannot be combined with locator fields. Upstream `find` has no `select` action; the wrapper never sends `find … select` to the binary.
