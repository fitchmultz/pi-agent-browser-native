...
- keep runtime cleanup ownership separate from branch-visible state: `session_tree` restore is serialized with managed-session work and may switch the visible branch, but extension-managed sessions and wrapper-launched Electron records owned by the current process remain eligible for quit/cleanup, and fresh-session allocation stays monotonic across branch restores
- when `/reload` shuts down an extension instance, close off-branch owned managed sessions before clearing process-local ownership; preserve only the current branch-visible active managed session for reload continuity, and rebuild that one from the active branch on the next `session_start`
