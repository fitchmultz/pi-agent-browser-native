...
If a compiled `semanticAction` fails with `failureCategory: "selector-not-found"`, visible content includes an `Agent-browser candidate fallbacks` block when safe candidates exist, and `details.nextActions` includes the normal `refresh-interactive-refs` snapshot step plus bounded locator-candidate fallbacks derived from the requested semantic target. For example, a missed `placeholder` fill may suggest role/name retries for `searchbox` and `textbox`, and a missed text click may suggest role/name retries for `button` and `link`. Candidate fallbacks are heuristics, not proof that an element exists; inspect the page if the target could be ambiguous.
