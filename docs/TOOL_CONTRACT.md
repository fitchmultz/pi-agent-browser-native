...
- `successCategory`: present on successful results. Current values are `"completed"`, `"artifact-saved"`, `"artifact-unverified"`, and `"inspection"`. `artifact-unverified` means upstream reported success but at least one non-pending artifact was not verified as present on disk; inspect `artifactVerification` before using the path.
...
- Success: if `inspection` is true → `"inspection"`; else if any non-pending artifact is missing or unverified → `"artifact-unverified"`; else if there is a `savedFile` or any `artifacts` → `"artifact-saved"`; else → `"completed"`.
...
- `artifactVerification` for a normalized per-result verification summary. It reports `verified`, `verifiedCount`, `missingCount`, `pendingCount`, `unverifiedCount`, and one entry per artifact or spill with `path`, optional `absolutePath`, optional `requestedPath`, `kind`, `mediaType`, `exists`, `sizeBytes`, `status`, `retentionState`, `storageScope`, and a `state` of `verified`, `missing`, `pending`, or `unverified`. The summary `verified` boolean is true only when every entry is verified. `record start` is `pending` until `record stop`; `state load` may mention a path in command output but is not a saved artifact.
