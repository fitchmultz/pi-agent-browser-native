...
- On Pi `quit`, active wrapper-owned Electron launches are best-effort cleaned. On `/reload`, current branch-visible active Electron launches are preserved for reload continuity while off-branch owned launches are cleaned before process-local ownership is cleared. Stale restored records are reported instead of guessed/killed when the wrapper lacks a live child process.
...
- when `/reload` shuts down an extension instance, close off-branch owned managed sessions and off-branch owned Electron launches before clearing process-local ownership; preserve only the current branch-visible active managed session and active Electron launch for reload continuity, and rebuild those from the active branch on the next `session_start`
