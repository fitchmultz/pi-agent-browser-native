...
- `selectorTextVisibility` after `get text <selector>` (or the first hazardous selector in a successful `batch`) when a non-ref CSS selector has multiple matches or a hidden first match while visible matches exist; includes selector, match counts, first-visible preview, and a summary warning that the upstream text may come from hidden/tabbed DOM content. If multiple batched selectors are hazardous, `selectorTextVisibilityAll` preserves every warning, sorted so hidden-first matches appear before general ambiguity warnings.
