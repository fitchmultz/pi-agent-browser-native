...
- `clickDispatch` when a top-level non-Electron `click` reported upstream success but no trusted DOM event reached the target and the wrapper replayed (or failed to replay) the same element activation; shape follows `ClickDispatchDiagnostic` in `extensions/agent-browser/lib/orchestration/browser-run/types.ts`
...
- `overlayBlockers` for conservative post-click overlay/banner/dialog blocker candidates when a direct click stays on the same URL, no click-dispatch fallback already applied, and a fresh snapshot provides evidence (`candidates`, `summary`, and `snapshot` per `OverlayBlockerDiagnostic` in `extensions/agent-browser/index.ts`)
