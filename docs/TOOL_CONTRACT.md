...
- **Batch guarded-ref scope for URL/membership preflight:** For `batch` stdin, `buildStaleRefPreflight` only collects `@e…` from guarded steps that appear **before** the first step whose first token is `snapshot` (`getGuardedRefUsage` in `extensions/agent-browser/index.ts`). That keeps pre-spawn URL drift and unknown-ref checks tied to the session’s last persisted `refSnapshot`; guarded steps after an in-plan `snapshot` row rely on upstream once that `snapshot` runs inside the batch (see `agentBrowserExtension allows batch stdin ref steps after snapshot following an invalidating step` in `test/agent-browser.extension-validation.test.ts`). Direct (non-`batch`) calls still evaluate every guarded `@e…` token on the argv list against the session snapshot.
