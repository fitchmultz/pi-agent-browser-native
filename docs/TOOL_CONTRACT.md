...
- `sessionRecoveryHint` when startup-scoped flags need `sessionMode: "fresh"` while an implicit session is already active: includes `reason`, `recommendedSessionMode` (`"fresh"`), redacted `exampleArgs`, and `exampleParams` with the same args plus `sessionMode: "fresh"` (from `buildExecutionPlan` in `extensions/agent-browser/lib/runtime.ts`, attached in `extensions/agent-browser/index.ts`)
