...
- Always close browser work with close/quit/exit when finished. The wrapper tracks managed sessions and caller explicit open/goto/navigate --session launches; it best-effort closes managed sessions on Pi quit and closes tracked explicit opens on every extension shutdown (quit or /reload, because reload starts a new instance). Idle timeout (default 15 minutes via AGENT_BROWSER_IDLE_TIMEOUT_MS / PI_AGENT_BROWSER_IMPLICIT_SESSION_IDLE_TIMEOUT_MS) is the crash backstop. pi-agent-browser-doctor warns about live /tmp/piab*.pid sidecars.
...
- `select` accepts either non-empty `selector` plus `value`/`values`, or a narrow locator form the wrapper resolves to exactly one current visible combobox/listbox `@ref` on an active session. Supported locator forms: `{ action: "select", locator: "role", role: "combobox"|"listbox", name: "Flavor", value: "chocolate" }` (or `values`) and `{ action: "select", locator: "label", value: "Flavor", values: ["chocolate"] }`. Other locators, non-combobox/listbox roles, and role select without `name` are rejected. `select` does not accept `text`, and `selector` cannot be combined with locator fields. Upstream `find` has no `select` action; the wrapper never sends `find … select` to the binary and never offers `retry-semantic-action-after-stale-ref` for select.
...
- optional `role` is accepted only when `locator` is `role`. For click/check/fill it may replace `value` and must equal `value` if both are set. For select, `role` must be `combobox` or `listbox`, `name` is required, and `value`/`values` are option values (not the role token).
...
| `select` + `locator` + option `value`/`values` | resolved on an active session to `["select",<@ref>,<value...>]` (compiled form keeps find-shaped locator metadata in `details.compiledSemanticAction`) |
| any supported action + `session` | prepends `["--session",<session>]` before the compiled argv |

When `semanticAction` compiles successfully, `details.compiledSemanticAction` echoes `{ action, locator, args }` for `find` actions, `{ action, selector, args }` for direct selector/ref click/check/fill actions, `{ action: "select", selector, values, args }` for selector select, or `{ action: "select", locator, values, args }` for locator select, with `args` redacted the same way as other invocation details. Expect it on the initial wrapper validation return (when that path still builds the early `details` object) and on the unified result after `agent-browser` runs. It is omitted when the call used `args` only, when compilation never produced argv, and on some in-`execute` error returns that attach a slimmer `details` shape before the unified merge (for example certain session-plan, stdin-contract, tab-pinning, or missing-binary guard paths); compare `extensions/agent-browser/index.ts` where `compiledSemanticAction` is assigned. For active sessions, role/name `click`, `check`, guarded `fill`, and locator `select` semantic actions may be resolved through one fresh `snapshot -i` to a current visible `@ref` before execution; fill only resolves when one exact editable `combobox`, `searchbox`, or `textbox` ref matches, and locator select only resolves when exactly one matching combobox/listbox ref is present. This avoids hidden duplicate matches stealing an upstream `find` action. In that case `details.compiledSemanticAction` still records the original semantic target while `details.effectiveArgs` shows the executed ref action.
...
- close the active extension-managed session when the originating `pi` process quits, and best-effort close wrapper-tracked explicit open/goto/navigate `--session` launches on every extension shutdown (quit or `/reload`)
...
- treat explicit caller-provided `--session` choices as day-to-day user-controlled names (not the default managed session). Successful open/goto/navigate through the tool still tracks those names for best-effort shutdown close. `--session` alone isolates a live browser session but is not a persisted tab/auth restore mechanism after a close command (`close`, `quit`, or `exit`), so use `--session <id> --restore`, `--profile`, or `--state` when persisted auth/tab state is required
