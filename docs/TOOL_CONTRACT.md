...
- treat explicit caller-provided non-managed `--session` choices as user-managed; those names alone isolate a live browser session but are not a persisted tab/auth restore mechanism after a close command (`close`, `quit`, or `exit`), so use `--session <id> --restore`, `--profile`, or `--state` when persisted auth/tab state is required. Wrapper-owned managed sessions (only when the extension injects/rotates the managed session, not merely because a name starts with `piab-`) set a cwd-stable `AGENT_BROWSER_RESTORE` env key automatically (disable with `PI_AGENT_BROWSER_MANAGED_SESSION_RESTORE=0`) so SSO cookies/localStorage/sessionStorage can survive idle shutdown and later chats in the same project without a manual `state save` flow. Any discovered project/user/explicit upstream config blocks automatic restore without parsing caller-selected content, as do browser mutation flags/env such as extensions, init scripts, raw launch args, custom user agents/executables, proxies, plugins, profiles, parent env namespaces, CDP, providers, state, and containment. After a wrapper-owned close succeeds, the wrapper retains the two newest snapshot families for its exact cwd-stable restore key and removes additional families older than 30 days without running namespace-wide `state clean`; caller-owned state files are untouched. On POSIX the wrapper enforces owner-only mode `0700` on `~/.agent-browser` before automatic restore; Windows requires a valid 64-character hex `AGENT_BROWSER_ENCRYPTION_KEY`. When a managed session was launched with an incompatible path, `details.managedSessionRestoreDisabled: true` means later bare follow-ups will not inject restore for that session identity. An incompatible call that targets an already-active restore-enabled managed session fails before spawn because its daemon can retain the restore key; close it first, retry without an explicit session using `sessionMode: "fresh"`, or use a distinct explicit session. Identity flags use upstream's separated forms (`--session <name>`, `--namespace <name>`); the wrapper rejects leading equals forms that upstream 0.33.2 does not recognize
