...
- **Managed restore security and persistence:** while the restore-enabled managed daemon is active, assert raw argument and stdin batches containing nested `connect` fail before upstream spawn; seed a cookie plus localStorage/sessionStorage, close the first managed browser, create a new extension harness with the same cwd, reopen the fixture, and assert all three values restore before closing the second browser. On POSIX, a separate isolated real-browser launch also asserts automatic restore stays disabled and no snapshot is written through a symlinked `sessions` directory, including during close.
