...
- **Default gate:** `npm run verify` runs the full unit suite under `test/**/*.test.ts`, which includes [`test/agent-browser.efficiency-benchmark.test.ts`](../test/agent-browser.efficiency-benchmark.test.ts) for scenario coverage and comparison behavior. It does not spawn the standalone benchmark script’s JSON/Markdown run; that is what the opt-in slice below adds.
