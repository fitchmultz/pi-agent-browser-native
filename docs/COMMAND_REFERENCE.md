...
Wrapper result rendering is metadata-first for saved files. An artifact-producing command fails as `artifact-missing` with artifact `status: "stale"` when the reported path's `mtimeMs` falls outside the command's bounded start/end window (with two seconds of filesystem precision tolerance); clearly old or future-dated evidence is never accepted as a fresh capture. A batch must use distinct explicit artifact destinations so one step cannot verify another step's file. `wait --download` is observational and may verify a download that completed just before the wait began, so it is exempt from the command-window mtime gate:
