...
When a direct `click` succeeds and the wrapper confirms the URL did not change, the wrapper may run one fresh `snapshot -i` and surface `Possible overlay blockers` plus `details.overlayBlockers` if that snapshot contains both overlay context and close/dismiss-like controls. Treat `inspect-overlay-state` as the safe first follow-up; only use a `try-overlay-blocker-candidate-*` next action when the candidate is clearly the blocker you intend to close.
