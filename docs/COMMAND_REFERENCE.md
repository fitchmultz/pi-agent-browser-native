...
| `wait --url <pattern>` | Wait for the URL to match a pattern. On timeout the wrapper prepends a `fresh-session-after-url-wait-timeout` next action (`sessionMode: "fresh"` + `snapshot -i`): if a preceding click or submit reported success but the page never navigated, upstream click dispatch may have silently missed, so replay the flow as one batch in a fresh session instead of retrying the wait. |
