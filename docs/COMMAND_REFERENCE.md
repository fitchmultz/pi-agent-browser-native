...
- This wrapper keeps its managed-session idle override (default 15 minutes via `PI_AGENT_BROWSER_IMPLICIT_SESSION_IDLE_TIMEOUT_MS` / `AGENT_BROWSER_IDLE_TIMEOUT_MS`) and enables cwd-stable `AGENT_BROWSER_RESTORE` for wrapper-owned managed sessions so SSO cookies survive browser relaunches. Caller-owned `--session` names (including ones that start with `piab-`) do not get that inject. Set `PI_AGENT_BROWSER_MANAGED_SESSION_RESTORE=0` to disable it. No Eve-specific Pi runtime is added.
...
- `--restore [name]`: auto-save/restore cookies, local storage, and session storage; bare `--restore` uses `--session` as the key. Environment: `AGENT_BROWSER_RESTORE`. Wrapper-owned managed sessions set a cwd-stable restore key automatically unless disabled with `PI_AGENT_BROWSER_MANAGED_SESSION_RESTORE=0` or sticky-disabled after an incompatible launch; a successfully spawned suppressed identity reports `details.managedSessionRestoreDisabled` and remains reusable without restore. Any discovered upstream config or browser mutation flag/env disables automatic managed restore without reading caller-selected config content. Raw batch argv and batch stdin containing nested `connect`/`batch` also disable restore so an attached browser cannot receive wrapper-managed auth state. Incompatible calls cannot reuse an active restore-enabled managed daemon; close it first, use a fresh wrapper session, or choose a distinct explicit session. Upstream restore files under `~/.agent-browser/` are plaintext unless a valid 64-character hex `AGENT_BROWSER_ENCRYPTION_KEY` is set; before automatic managed restore the wrapper enforces POSIX mode `0700` and rejects symlinks/non-directories along the exact restore `sessions` path plus symlink entries inside that directory, while Windows requires that key because POSIX mode checks cannot verify profile ACLs. After a wrapper-owned close succeeds, the wrapper retains the two newest snapshot families for its exact cwd-stable restore key and removes additional families older than 30 days without invoking namespace-wide `state clean`; caller-owned saved-state files are untouched. Use `state clear` / `state clean` for immediate cleanup.
