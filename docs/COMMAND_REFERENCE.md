...
- `batch` keeps each step's artifacts in `details.batchSteps[].artifacts`; top-level `details.artifacts` preserves step order but coalesces an earlier pending recording into the later saved, missing, or stale terminal result for the same session
