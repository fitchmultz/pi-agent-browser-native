...
Wrapper result rendering is metadata-first for saved files. A requested path that already existed but whose `mtimeMs` predates the current command fails as `artifact-missing` with artifact `status: "stale"` and `updatedAtMs`; old evidence is never accepted as a fresh capture. `wait --download` is observational and may verify a download that completed just before the wait began, so it is exempt from the command-start mtime gate:
