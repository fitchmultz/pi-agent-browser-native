...
- for extension-managed `piab-*` sessions, also set a cwd-stable `AGENT_BROWSER_RESTORE` key on every upstream subprocess so cookies, localStorage, and sessionStorage autosave/restore across idle shutdowns and later Pi chats in the same project directory; skip when the caller already set restore/profile/state/cdp/auto-connect/allowed-domains/session-name via argv or the matching `AGENT_BROWSER_*` parent env vars (including legacy `AGENT_BROWSER_SESSION_NAME` and `AGENT_BROWSER_CDP`), when the command is `connect`, or when `PI_AGENT_BROWSER_MANAGED_SESSION_RESTORE=0`. Once a managed session hits any of those incompatible launch paths, restore stays disabled for later bare follow-ups on that same session name (persisted as `details.managedSessionRestoreDisabled` for resume/reload). This is env-based persistence, not a hidden argv relaunch. Explicit non-`piab-*` `--session` names stay caller-owned without automatic restore. Upstream still owns restore file paths/modes under `~/.agent-browser/`; set `AGENT_BROWSER_ENCRYPTION_KEY` on multi-user hosts if plaintext session files are unacceptable. Project `agent-browser.json` launch defaults are not parsed by this skip check—disable managed restore or pass conflicting flags explicitly when those defaults own auth/containment.
...
Upstream restore-state persistence remains upstream-owned. The wrapper passes `AGENT_BROWSER_AUTOSAVE_INTERVAL_MS` through unchanged and may set `AGENT_BROWSER_RESTORE` for managed `piab-*` sessions as described above; the behavior introduced in upstream 0.31.2 periodically saves restore-enabled cookies/localStorage after command quiet time and during idle page changes, while `0` disables periodic saves without disabling save-on-close. The wrapper must not duplicate that timer or treat upstream restore files as wrapper-owned artifacts.
