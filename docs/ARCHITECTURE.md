...
- inject `--json`
- complete each upstream invocation when the direct `agent-browser` child exits even if Node delays the child `"close"` event: piped stdio can stay referenced by longer-lived descendant processes, so `runAgentBrowserProcess` watches `exit` and `close` together and applies a short post-`exit` grace before resolving with the observed exit code (`watchSpawnedChildCompletion` in `extensions/agent-browser/lib/process.ts`) so the tool cannot hang after the CLI process has already terminated
