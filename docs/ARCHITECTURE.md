...
- for wrapper-owned managed sessions only, also set a cwd-stable `AGENT_BROWSER_RESTORE` key on every upstream subprocess so cookies, localStorage, and sessionStorage autosave/restore across idle shutdowns and later Pi chats in the same project directory. Policy lives in `extensions/agent-browser/lib/managed-session-restore.ts`; ownership is resolved by `resolveOwnedManagedSessionContext` (injected managed session, or explicit `--session` equal to the current managed name and namespace) and applied through `AsyncLocalStorage` `withOwnedManagedSessionContext` for prepare helpers plus main process/output, with typed `ownedManagedSession` process options for owned main/close spawns rather than an internal marker leaked into the child environment. `buildOwnedManagedSessionRestoreContext` sets call-scoped `restoreSuppressed` from main-plan argv so helper probes skip restore on incompatible plans without sticky-disabling when prepare returns early; sticky disable commits only when an owned main/close spawn actually runs incompatible argv, while abandoned failed fresh-session identities are cleared. Duplicate `--session` or `--namespace` flags are rejected, as are leading equals forms that upstream 0.33.2 does not recognize; global identity/config scanning follows upstream across the full argv rather than treating `--` as a sentinel. Ownership is not inferred from a `piab-*` name prefix: explicit caller `--session` values, including names that happen to start with `piab-`, stay caller-owned without automatic restore, including helper probes against those sessions. Skip when the caller already set restore/profile/state/CDP/provider/auto-connect/containment/session-name or a browser mutation surface (custom executable, extension, init script, raw launch args, proxy, plugin, or related engine/device controls) via argv or matching parent env, when the command is `connect`, or when `PI_AGENT_BROWSER_MANAGED_SESSION_RESTORE=0`. The wrapper's own ChatGPT headless user-agent compatibility injection is excluded from caller-mutation policy. An incompatible call targeting an already-active restore-enabled managed identity fails before spawn because the live upstream daemon can retain its restore key; close it first, use a fresh wrapper session, or choose a distinct explicit session. Once a managed session hits any allowed incompatible launch path, restore stays disabled for later bare follow-ups on that same session identity. Sticky identities live in the extension-owned `ManagedSessionRestoreState` instance, persist as `details.managedSessionRestoreDisabled`, and are replaced from current-branch rows during branch restore rather than stored in module-global process state. The opt-out returns before config/storage probes while still sticky-recording explicit incompatible launch plans. This is env-based persistence, not a hidden argv relaunch. Upstream still owns restore file paths/modes under `~/.agent-browser/`; set `AGENT_BROWSER_ENCRYPTION_KEY` on multi-user hosts if plaintext session files are unacceptable. Any discovered project `./agent-browser.json`, explicit `--config` / `AGENT_BROWSER_CONFIG`, or `~/.agent-browser/config.json` disables managed restore without reading caller-selected content in the Pi host; owned spawns sticky-disable that session identity. Because upstream writes a snapshot per daemon session, successful wrapper-owned closes prune only snapshot families older than 30 days for the exact cwd-stable `piab-r-*` key while retaining the two newest families; the wrapper never invokes upstream's namespace-wide `state clean` and therefore leaves caller-owned state untouched. Upstream restore files under `~/.agent-browser/` remain plaintext unless `AGENT_BROWSER_ENCRYPTION_KEY` is set; before automatic managed restore the wrapper enforces POSIX mode `0700` on that parent directory and fails closed if it cannot, while Windows requires the documented 64-character hex encryption key because POSIX mode checks cannot verify profile ACLs; malformed keys fail closed on every platform.
