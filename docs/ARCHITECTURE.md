...
- for wrapper-owned managed sessions only, also set a cwd-stable `AGENT_BROWSER_RESTORE` key on every upstream subprocess so cookies, localStorage, and sessionStorage autosave/restore across idle shutdowns and later Pi chats in the same project directory. Ownership is resolved by `resolveOwnedManagedSessionContext` (injected managed session, or explicit `--session` equal to the current managed name and namespace) and applied through `AsyncLocalStorage` `withOwnedManagedSessionContext` for prepare helpers plus main process/output, with `PI_AGENT_BROWSER_OWNED_MANAGED_SESSION=1` on owned main/close `options.env` only (not trusted from parent env). `applyManagedSessionRestorePlanPolicy` sets call-scoped `restoreSuppressed` from main-plan argv so helper probes skip restore on incompatible plans without sticky-disabling when prepare returns early; sticky disable commits only when an owned main/close spawn actually runs incompatible argv. Multiple `--session` flags are rejected. Ownership is not inferred from a `piab-*` name prefix: explicit caller `--session` values, including names that happen to start with `piab-`, stay caller-owned without automatic restore, including helper probes against those sessions. Skip when the caller already set restore/profile/state/cdp/provider/auto-connect/allowed-domains/session-name via argv or the matching `AGENT_BROWSER_*` parent env vars (including legacy `AGENT_BROWSER_SESSION_NAME`, `AGENT_BROWSER_CDP`, and `AGENT_BROWSER_PROVIDER`), when the command is `connect`, or when `PI_AGENT_BROWSER_MANAGED_SESSION_RESTORE=0`. Once a managed session hits any of those incompatible launch paths, restore stays disabled for later bare follow-ups on that same session identity (persisted as `details.managedSessionRestoreDisabled` for resume/reload, including explicit current-managed rows). This is env-based persistence, not a hidden argv relaunch. Upstream still owns restore file paths/modes under `~/.agent-browser/`; set `AGENT_BROWSER_ENCRYPTION_KEY` on multi-user hosts if plaintext session files are unacceptable. Project `./agent-browser.json`, `AGENT_BROWSER_CONFIG`, and `~/.agent-browser/config.json` are shallow-parsed for incompatible launch defaults (`cdp`, `provider`, `profile`, `state`, `restore`, `allowedDomains`/`allowed_domains`, `autoConnect`/`auto_connect`, `sessionName`/`session_name`); when any are set, managed restore is skipped and sticky-disabled on owned spawns. Upstream restore files under `~/.agent-browser/` remain plaintext unless `AGENT_BROWSER_ENCRYPTION_KEY` is set; before automatic managed restore the wrapper enforces POSIX mode `0700` on that parent directory and fails closed if it cannot, while Windows requires the encryption key because POSIX mode checks cannot verify profile ACLs.
