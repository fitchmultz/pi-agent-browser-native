...
- inject `--json`
- support optional stdin only for `eval --stdin`, `batch`, `auth save --password-stdin`, and wrapper-generated `batch` stdin from top-level `job`, `qa`, or `sourceLookup`, rejecting other command/stdin combinations before launch
