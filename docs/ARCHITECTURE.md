...
- compact result summaries, including presentation-time redaction for stateful browser-context commands (`auth`, `cookies`, `storage`, `dialog`, `frame`, `state`): model-facing `details.data` and batch roll-ups avoid expanding secret-bearing payloads, `redactInvocationArgs` in `extensions/agent-browser/lib/runtime.ts` masks positional values for `cookies set` and `storage local|session set`, and nested `batch` steps use the same argv and error-body scrubbing before echoing commands or errors (see `extensions/agent-browser/lib/results/presentation.ts`)
