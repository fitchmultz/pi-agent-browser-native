...
- close the active extension-managed session when the originating `pi` process quits, and close wrapper-tracked explicit open/goto/navigate sessions on every extension shutdown (quit or reload)
...
- keep process-owned cleanup registries for extension-managed sessions, wrapper-opened explicit sessions (successful open/goto/navigate with caller `--session`, closed on every extension shutdown including `/reload` because reload loads a new instance), and wrapper-launched Electron records separate from the current branch-visible view; `session_tree` restore and wrapper-owned browser commands are serialized with managed-session work, while independent explicit-session commands keep their parallel tab-target behavior (explicit cleanup ownership does not force the managed serialize queue) but use a branch-state generation guard so stale completions cannot overwrite newer branch-visible managed/artifact state after a branch switch; branch switches still must not drop resources the current Pi process owns and must keep fresh-session allocation monotonic
...
- leave explicit caller-provided `--session` choices alone for day-to-day control, but best-effort close wrapper-tracked explicit opens when the extension instance shuts down (quit or reload)
