...
- keep process-owned cleanup registries for extension-managed sessions and wrapper-launched Electron records separate from the current branch-visible view; `session_tree` restore is serialized with managed-session work and can replace the visible branch state, but it must not drop resources the current Pi process still owns and must keep fresh-session allocation monotonic
- on non-quit shutdown such as `/reload`, close off-branch owned managed sessions before clearing process-local ownership, but preserve the current branch-visible active managed session so reload continuity still works from the active transcript branch
