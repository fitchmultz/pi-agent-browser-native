...
- for extension-managed `piab-*` sessions, also set a cwd-stable `AGENT_BROWSER_RESTORE` key on every upstream subprocess so cookies and localStorage autosave/restore across idle shutdowns and later Pi chats in the same project directory; skip when the caller already set restore/profile/state/cdp/auto-connect/allowed-domains, when the command is `connect`, or when `PI_AGENT_BROWSER_MANAGED_SESSION_RESTORE=0`. This is env-based persistence, not a hidden argv relaunch. Explicit non-`piab-*` `--session` names stay caller-owned without automatic restore
