...
- close the active extension-managed session and any wrapper-tracked explicit open/goto/navigate sessions when the originating `pi` process quits
...
- keep process-owned cleanup registries for extension-managed sessions, wrapper-opened explicit sessions (successful open/goto/navigate with `--session`, quit-only close), and wrapper-launched Electron records separate from the current branch-visible view; `session_tree` restore and wrapper-owned browser commands are serialized with managed-session work, while independent explicit-session commands keep their parallel tab-target behavior (explicit cleanup ownership does not force the managed serialize queue) but use a branch-state generation guard so stale completions cannot overwrite newer branch-visible managed/artifact state after a branch switch; branch switches still must not drop resources the current Pi process owns and must keep fresh-session allocation monotonic
...
- leave explicit caller-provided `--session` choices alone for day-to-day control (no auto-close on reload), but best-effort close wrapper-tracked explicit opens on Pi quit
