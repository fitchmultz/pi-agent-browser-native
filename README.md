...
- Do not treat an arbitrary `--session` name alone as persisted auth after `close`, `quit`, or `exit`. Wrapper-owned managed sessions automatically set a cwd-stable `AGENT_BROWSER_RESTORE` key so cookies/localStorage/sessionStorage survive browser relaunches across Pi chats in the same project; a bare caller `--session` name does not get that injection even if it starts with `piab-`. Disable with `PI_AGENT_BROWSER_MANAGED_SESSION_RESTORE=0`. For explicit non-managed sessions use `--session <id> --restore`, `--profile`, or `--state`. SSO/2FA such as Okta Touch ID may still need one human approval (often `--headed` the first time); after that, managed restore should keep the session without a manual `state save` dance. Any discovered upstream `agent-browser.json` / `AGENT_BROWSER_CONFIG` / `--config`, raw batch argv, batch stdin containing nested `connect`/`batch`, browser mutation flag, or matching launch-mutation env disables automatic managed restore rather than risking restored auth in a caller-customized or attached browser. The wrapper rejects incompatible calls that target an already-active restore-enabled managed session because its live daemon can retain the restore key; close it first, omit the explicit session and use `sessionMode: "fresh"`, or choose a distinct explicit session. Upstream restore files live under `~/.agent-browser/` and are plaintext unless you set `AGENT_BROWSER_ENCRYPTION_KEY`; on POSIX the wrapper enforces mode `0700` and rejects symlinks/non-directories along the exact restore `sessions` path plus symlink entries inside that directory before automatic managed restore, and fails closed if it cannot. Windows automatic managed restore requires the documented 64-character hex `AGENT_BROWSER_ENCRYPTION_KEY` because POSIX mode checks cannot verify profile ACLs. After a wrapper-owned managed session closes successfully, the wrapper keeps the two newest snapshot families for its exact cwd-stable restore key and removes additional families older than 30 days; it never invokes upstream's namespace-wide `state clean`, so caller-owned state files are untouched. Upstream periodically saves restore-enabled cookies/localStorage while the browser is open; `AGENT_BROWSER_AUTOSAVE_INTERVAL_MS` defaults to `30000`, `0` disables periodic saves but keeps save-on-close, and the `never` value for `--restore-save` disables automatic saves for that restore session.
