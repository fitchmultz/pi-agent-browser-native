...
- If a direct click succeeds and the URL stays put, visible text may add `Possible overlay blockers` with `details.overlayBlockers` and session-aware `inspect-overlay-state` / `try-overlay-blocker-candidate-*` next actions, but only when a fresh snapshot provides overlay/banner/dialog evidence and a close-like control.
