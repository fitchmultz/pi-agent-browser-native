"use strict";
(() => {
  // chrome-relay/extension/background.ts
  ...
  var pingTimer = null;
  var relayInitiatedDetachTabs = /* @__PURE__ */ new Set();
  function sanitizeHost(raw) {
    return typeof raw === "string" && /^[A-Za-z0-9._-]+$/.test(raw) ? raw : DEFAULT_HOST;
  }
  async function loadSettings() {
    const stored = await chrome.storage.local.get({ host: DEFAULT_HOST, port: DEFAULT_PORT, token: "" });
    const port = Number(stored.port);
    return {
      host: sanitizeHost(stored.host),
      port: Number.isInteger(port) && port > 0 && port <= 65535 ? port : DEFAULT_PORT,
      token: typeof stored.token === "string" ? stored.token : ""
    };
  }
  function snapshot(tab) {
    if (tab.id === void 0) return null;
    return {
      tabId: tab.id,
  ...
  async function groupTabs(tabIds, title, color) {
    ...
    for (const tabId of tabIds) {
      try {
        ...
        if (tab.pinned || tab.id === void 0) continue;
    ...
    for (const [windowId, ids] of byWindow) {
      ...
      if (existing[0]) {
        ...
        for (const dupe of existing.slice(1)) {
          ...
          const dupeIds = dupeTabs.map((tab) => tab.id).filter((id) => id !== void 0);
  ...
  async function buildHello() {
    ...
    for (const target of targets) {
      if (target.attached && target.tabId !== void 0) attachedTabIds.push(target.tabId);
    }
  ...
  function handleRelayMessage(raw) {
    ...
    void runRpc(msg).then((result) => post({ t: "rpcResult", id: msg.id, ok: true, result })).catch((err) => {
  ...
  async function connect() {
    ...
    const url = `ws://${settings.host}:${settings.port}/ext${settings.token ? `?token=${encodeURIComponent(settings.token)}` : ""}`;
    ...
    socket.onopen = () => {
      ...
      clearInterval(pingTimer ?? void 0);
