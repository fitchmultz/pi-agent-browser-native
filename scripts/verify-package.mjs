 ...
 * Responsibilities: Run `npm pack --json --dry-run`, validate required and forbidden packed files, confirm local development-only files stay unpacked, and print a concise release report.
...
const REQUIRED_REPO_FILES = [".pi/extensions/agent-browser.ts", "LICENSE"];
...
Checks:
  1. Required repo-only files exist (for example the tracked .pi dev entrypoint).
