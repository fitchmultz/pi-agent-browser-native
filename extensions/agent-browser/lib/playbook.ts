...
export const QUICK_START_GUIDELINES = [
	"Quick start mental model: use exactly one of args (exact agent-browser CLI args after the binary), semanticAction (a thin find-locator shorthand compiled to find argv), or job (a constrained short-workflow schema compiled to batch); stdin is only for batch, eval --stdin, auth save --password-stdin, and wrapper-generated job batches, and other command/stdin combinations are rejected before launch; sessionMode=fresh switches the extension-managed pi-scoped session to a fresh upstream launch when you need new --profile, --session-name, --cdp, --state, --auto-connect, --init-script, --enable, -p/--provider, or iOS --device state.",
...
export const TOOL_PROMPT_GUIDELINES_SUFFIX = [
	...
	"Pass exact agent-browser CLI arguments in args when you are not using semanticAction or job, excluding the binary name.",
	"Use stdin only for eval --stdin, batch, auth save --password-stdin, or wrapper-generated job batches instead of shell heredocs or password args; other command/stdin combinations are rejected before launch.",
