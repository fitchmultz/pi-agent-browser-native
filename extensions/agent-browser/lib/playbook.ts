...
export const QUICK_START_GUIDELINES = [
	"Quick start mental model: use exactly one of args (exact agent-browser CLI args after the binary), semanticAction (a thin find-locator shorthand compiled to find argv), job (a constrained short-workflow schema compiled to batch), or qa (a lightweight QA preset built on job/batch); stdin is only for batch, eval --stdin, auth save --password-stdin, and wrapper-generated job/qa batches, and other command/stdin combinations are rejected before launch; sessionMode=fresh switches the extension-managed pi-scoped session to a fresh upstream launch when you need new --profile, --session-name, --cdp, --state, --auto-connect, --init-script, --enable, -p/--provider, or iOS --device state.",
