...
export function createAgentBrowserParamsSchema(
	...
	semanticAction: Type.Optional(
		Type.Object({
			...
			locator: Type.Optional(StringEnum(AGENT_BROWSER_SEMANTIC_LOCATORS, {
				description: "Locator family for check/click/fill find actions, or for select when resolving a current combobox/listbox @ref (not combined with selector).",
			})),
			value: Type.Optional(Type.String({ description: "Locator value for find actions; for select+selector a single option value; for select+locator=role with role set, a single option value; for other select+locator forms, the control locator text (options go in values)." })),
	...
	sessionMode: Type.Optional(
		StringEnum(["auto", "fresh"] as const, {
			description:
				"Session handling mode. `auto` reuses the extension-managed pi-scoped session when possible. `fresh` switches that managed session to a fresh upstream launch so launch-scoped flags like --namespace, --restore, --restore-save, restore check flags, --profile, --executable-path, --webgpu, --session-name, --cdp, --state, --auto-connect, --init-script, --enable, -p/--provider, or iOS --device apply and later auto calls follow the new browser.",
