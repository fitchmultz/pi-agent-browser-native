...
export async function processBrowserOutput(input: ProcessBrowserOutputInput): Promise<BrowserProcessOutputResult> {
	...
	try {
		...
		if (succeeded && prepared.clickDispatchProbe) {
			...
			if (clickDispatchDiagnostic?.status === "fallback-failed") {
				succeeded = false;
		...
		if (succeeded && !sessionTabCorrection && !aboutBlankSessionMismatch && !electronRecordForCommand && clickDispatchDiagnostic?.status !== "fallback-applied") overlayBlockerDiagnostic = await collectOverlayBlockerDiagnostic({ command: prepared.executionPlan.commandInfo.command, cwd, data: presentationEnvelope?.data, navigationSummary, priorTarget: prepared.priorSessionTabTarget, sessionName: prepared.executionPlan.sessionName, signal });
		if (succeeded) {
