...
import { createFreshSessionName, extractUpstreamCommandTokens, isSuccessfulPlainTextOutput, resolveManagedSessionState } from "../../runtime.js";
...
export async function processBrowserOutput(input: ProcessBrowserOutputInput): Promise<BrowserProcessOutputResult> {
	...
	try {
		...
		const plainTextOutput = prepared.executionPlan.plainTextOutput === true && isSuccessfulPlainTextOutput(prepared.executionPlan.commandInfo.command, processSucceeded);
		const parseSucceeded = plainTextInspection || plainTextOutput || parseError === undefined;
		...
		const envelopeSuccess = plainTextInspection || plainTextOutput ? true : presentationEnvelope?.success !== false;
		let succeeded = processSucceeded && parseSucceeded && envelopeSuccess;
		const inspectionText = plainTextInspection || plainTextOutput ? processResult.stdout.trim() : undefined;
		...
		const plannedPlainTextOutput = prepared.executionPlan.plainTextOutput;
		let errorText = getAgentBrowserErrorText({ aborted: processResult.aborted, command: prepared.executionPlan.commandInfo.command, effectiveArgs: prepared.redactedProcessArgs, envelope: presentationEnvelope, exitCode: processResult.exitCode, parseError, plainTextInspection, plainTextOutput: plannedPlainTextOutput, staleRefArgs: getStaleRefArgs(prepared.commandTokens, prepared.runtimeToolStdin), spawnError: processResult.spawnError, stderr: processResult.stderr, stdout: processResult.stdout, timedOut: processResult.timedOut, timeoutMs: processResult.timeoutMs, wrapperRecoveryHint: buildWrapperRecoveryHint({ pinnedBatchUnwrapMode: prepared.pinnedBatchUnwrapMode, sessionTabCorrection }) });
		if (errorText) {
		...
		const redactedContent = buildRedactedPresentationContent({ exactSensitiveValues: prepared.exactSensitiveValues, plainTextInspection: plainTextInspection || plainTextOutput, presentation, presentationEnvelope, succeeded, userRequestedJson: prepared.userRequestedJson, warningText });
		const finalRecoveryState = await prepareFinalResultRecoveryState({ aboutBlankSessionMismatch, batchRefSnapshotState, commandTokens: prepared.commandTokens, compiledSemanticAction: prepared.compiledSemanticAction, currentRefSnapshot, currentRefSnapshotInvalidation, currentSessionTabTarget, cwd, electronPostCommandHealth, errorText, executionPlan: prepared.executionPlan, parseError, plainTextInspection: plainTextInspection || plainTextOutput, presentation, processResult, redactedProcessArgs: prepared.redactedProcessArgs, runtimeToolArgs: prepared.runtimeToolArgs, sessionPageState, sessionPageStateUpdate, sessionTabCorrection, signal, succeeded });
