...
export default function agentBrowserExtension(pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		promptGuidelines: [
			...
			"If a session lands on the wrong page or tab, or an interaction changes origin unexpectedly, use tab list / tab <n> / snapshot -i. Only use wait with an explicit argument like milliseconds, --load, --url, --fn, or --text.",
			"If an authenticated open returns a blocked-navigation or blank-title result, inspect the current tabs before changing strategies.",
			"For feed, timeline, or inbox reading tasks, focus on the main timeline/list region and read the first item there rather than unrelated composer or sidebar content.",
			"For read-only browsing tasks, prefer extracting the answer from the current snapshot, structured ref labels, or eval --stdin on the current page before navigating away. Only click into media viewers, detail routes, or new pages when the current view does not contain the needed information.",
			"When using eval --stdin for extraction, return the value you want instead of relying on console.log as the primary result channel.",
