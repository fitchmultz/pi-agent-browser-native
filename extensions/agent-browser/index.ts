...
const AGENT_BROWSER_PARAMS = Type.Object({
	...
	stdin: Type.Optional(Type.String({ description: "Optional raw stdin content; only supported for batch, eval --stdin, auth save --password-stdin, and is generated by job mode." })),
...
export default function agentBrowserExtension(pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			...
			const explicitInputModes = [hasExplicitArgs, Boolean(semanticActionResult.compiled), Boolean(jobResult.compiled)].filter(Boolean).length;
			...
			const inputModeError = explicitInputModes !== 1
				? "Provide exactly one of args, semanticAction, or job."
				: undefined;
			const compiledSemanticAction = semanticActionResult.compiled;
			const compiledJob = jobResult.compiled;
			...
			const jobStdinError = compiledJob && params.stdin !== undefined ? "Do not provide stdin with job; job mode generates its own batch stdin." : undefined;
			const validationError = semanticActionError ?? jobError ?? inputModeError ?? jobStdinError ?? validateToolArgs(toolArgs) ?? getBatchAnnotateValidationError(toolArgs, toolStdin);
