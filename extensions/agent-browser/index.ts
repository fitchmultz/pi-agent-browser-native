...
export default function agentBrowserExtension(pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		promptGuidelines: [
			...
			"For authenticated or user-specific content like feeds, inboxes, dashboards, and accounts, prefer --profile Default together with an explicit --session. Use --auto-connect only if profile-based reuse is unavailable or the task is specifically about attaching to a running debug-enabled browser.",
			...
			"If a profile session lands on the wrong page or tab, use tab list / tab <n> / snapshot -i. Only use wait with an explicit argument like milliseconds, --load, --url, --fn, or --text.",
			"For feed, timeline, or inbox reading tasks, focus on the main timeline/list region and read the first item there rather than unrelated composer or sidebar content.",
