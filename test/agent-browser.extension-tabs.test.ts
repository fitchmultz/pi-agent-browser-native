 ...
 * Usage: Run this file alone with `npx tsx --test test/agent-browser.extension-tabs.test.ts` (the `npm test` script uses a glob, so `npm test -- <path>` still runs the full suite). Full gate: `npm run verify`.
