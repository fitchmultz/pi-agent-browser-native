...
test("normalizePiVersion strips an optional pi binary label", () => {
	assert.equal(normalizePiVersion("pi 0.80.6\n"), "0.80.6");
	assert.equal(normalizePiVersion("0.80.6\n"), "0.80.6");
});
...
test("doctor fails when Pi is below the minimum runtime floor", async () => {
	...
	assert.equal(report.failures.length, 1);
	assert.match(text, /Pi 0\.80\.6 or newer is required; found 0\.80\.5/);
	assert.match(text, /enforces the Pi 0\.80\.6 runtime floor/);
...
test("doctor warns instead of failing when Pi version cannot be inspected", async () => {
	...
	assert.match(text, /Pi 0\.80\.6 or newer is required/);
