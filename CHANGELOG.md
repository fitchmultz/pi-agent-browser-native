...
- Wrapper-owned managed sessions now set a cwd-stable `AGENT_BROWSER_RESTORE` key so SSO cookies/localStorage/sessionStorage survive browser relaunches across Pi chats in the same project. Restore is ownership-gated (not inferred from a `piab-*` name alone), sticky-disabled after incompatible launches/config (profile/cdp/state/provider/allowed-domains/etc.), and opt-out with `PI_AGENT_BROWSER_MANAGED_SESSION_RESTORE=0`. The wrapper enforces owner-only POSIX mode `0700` on the upstream state parent directory before automatic restore and fails closed when it cannot; Windows requires `AGENT_BROWSER_ENCRYPTION_KEY`.
