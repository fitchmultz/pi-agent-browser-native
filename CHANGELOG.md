...
- Bare, review-only, and directly negated inbound image/video paths no longer become requested output artifacts that block browser close; output enforcement now requires a direct artifact-creation phrase with a destination, carries that intent across contiguous path-list lines, preserves subordinate requirements such as “do not close until you save,” scopes optional-recording qualifiers to their list, and scans large path lists once instead of once per path.
