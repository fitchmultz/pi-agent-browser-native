...
- Correct QA baseline-match warnings: matching error rows are not proven stale (#121). Add a real-browser rollover reproduction and document the native `errors --clear`/event-identity limitation and isolated-browser recovery; the upstream false-pass cause remains unresolved.
