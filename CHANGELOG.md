...
- Artifact verification rejects clearly old or future-dated output paths outside a bounded command window as `status: "stale"`, tolerates coarse filesystem mtimes, and rejects canonical or symlink-aliased duplicate explicit batch destinations; recording start/restart reports `artifact-pending` with an exact `stop-pending-recording` action. Observational `wait --download` still accepts a file that completed just before waiting began.
