...
- Final merge review also made Electron host launch cancellation no-spawn/cleanup-safe, decoupled managed daemon inspection from shorter caller watchdog overrides, recorded a null daemon policy after an inactive sticky-disabled session is restarted without restore, made POSIX daemon socket storage fail closed unless it is an absolute non-symlink current-user mode-`0700` directory, and blocked browser access to `.agent-browser` local state through encoded file targets, local-directory file interactions/refs, batches, and later capture from a tracked protected target.
...
- Passed `npm run verify -- pre-pr` (670 tests passed, 2 opt-in skips; 123 packed files), real-upstream contract, dogfood, packaged Pi, startup-profile, configured-source lifecycle, and local platform-target verification. The remote Crabbox macOS/Ubuntu/native-Windows matrix was unavailable and explicitly waived for this release.
