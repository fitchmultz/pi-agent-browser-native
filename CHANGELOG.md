...
### Added
- managed-session outcome diagnostics for failed `sessionMode: "fresh"` calls (`RQ-0077`): failed or timed-out fresh launches now report `details.managedSessionOutcome` and visible `Managed session outcome` text so agents know whether the prior managed session was preserved or no managed session became current.
