...
- Wrapper-owned managed sessions now set a cwd-stable `AGENT_BROWSER_RESTORE` key so SSO cookies/localStorage/sessionStorage survive browser relaunches across Pi chats in the same project. Restore is ownership-gated (not inferred from a `piab-*` name alone), sticky-disabled after incompatible launches (profile/cdp/state/provider/allowed-domains/etc.), and opt-out with `PI_AGENT_BROWSER_MANAGED_SESSION_RESTORE=0`. Upstream restore files remain plaintext unless `AGENT_BROWSER_ENCRYPTION_KEY` is set.
